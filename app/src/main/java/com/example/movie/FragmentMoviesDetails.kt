...
class FragmentMoviesDetails : Fragment() {
    ...
    override fun onCreateView(
        ...
        val movieId = arguments?.getInt("id_movie")
        ...
        uiScope.launch {
            ...
            val movie = withContext(Dispatchers.IO) {
                movieId?.let { context?.let { it1 -> movieRepository.findMovieById(it1, it) } }
            }

            val actors = withContext(Dispatchers.IO) {
                movie?.actors?.let {
                    actorRepository?.findActorsByMovieId(it)
                }
            }
            ...
            if (movie != null) {
                ...
                val genreNames =
                    withContext(Dispatchers.IO) { genreRepository?.findGenreNamesByIds(genreIds) }
